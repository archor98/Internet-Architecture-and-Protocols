...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    nxt_pack = self.new_packet(False, None, True)
    nxt_pack.tcp.seq = self.snd.iss
    self.state = SYN_SENT
    self.tx(nxt_pack)
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    ## End of Stage 1 ##
      ...
      if self.rx_queue.empty() or self.rcv.nxt |LT| self.rx_queue.peek()[0]:
        ...
        ##
         ...
         while (self.rx_queue.empty() == False) and (self.rcv.nxt |GE| self.rx_queue.peek()[0]):
                           arr = []
                           sequence_num, ip_packet_info = self.rx_queue.pop() #get sequence number and ip packet
                           seg = ip_packet_info.tcp
  ...
  def handle_synsent (self, seg):
    ...
    if acceptable_ack:
      ...
      increment = 1
      self.rcv.nxt = seg.seq | PLUS | increment
  ...
  def update_rto(self, acked_pkt):
    ...
    ## Start of Stage 9 ##
    cur_rtt = self.stack.now - acked_pkt.tx_ts
    count = self.K * self.rttvar
    if  self.rttvar == 0 and self.rto == 1 and self.srtt == 0:
      self.srtt = cur_rtt
      self.rttvar = cur_rtt / 2
    else:
      self.rttvar = (1 - self.beta) * self.rttvar + self.beta * abs(self.srtt - cur_rtt)
      self.srtt = (1 - self.alpha) * self.srtt + self.alpha * cur_rtt
    self.rto = self.srtt + max(self.G, self.K * self.rttvar)
    
    bol_1 = self.rto > self.MAX_RTO
    bol_2 = self.rto < self.MIN_RTO
    if bol_1 or bol_2:
        if bol_1:
  ...
  def handle_accepted_ack(self, seg):
    ...
    ## Start of Stage 8 ##
    acked_packets = self.retx_queue.pop_upto(seg.ack)
    ## End of Stage 8 ##


    ## Start of Stage 9 ##
    for (ackno, p) in acked_packets:
      if not p.retxed:
        self.update_rto(p)
  ...
  def handle_accepted_fin(self, seg):
    ...
    ## Start of Stage 7 ##
    if self.state in FIN_WAIT_1 and self.fin_ctrl.acks_our_fin(seg.ack):
        self.start_timer_timewait()
    if self.state in FIN_WAIT_1 and not self.fin_ctrl.acks_our_fin(seg.ack):
        self.state = CLOSING
  ...
  def maybe_send(self):
    ...
    num_pkts = 0
    window = snd.wnd
    bytes_sent = snd.nxt |MINUS| snd.una
    remaining_size = len(self.tx_data)

    ## Start of Stage 4 ##
    remaining = len(self.tx_data)
    while remaining > 0 and bytes_sent |LE| window:
        remaining_window = window - bytes_sent
        to_be_sent = min(self.mss, remaining_window,remaining_size)
        payload = self.tx_data[:to_be_sent]
        packet_new = self.new_packet(data=payload)
        self.tx(packet_new)
        self.tx_data = self.tx_data[to_be_sent:]
        remaining -= len(payload)
        num_pkts += 1
        bytes_sent += len(packet_new)
